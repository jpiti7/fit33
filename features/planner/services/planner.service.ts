...
import type { PlannedWorkout, WeeklyPlan } from "@/features/planner/types";
...
export function buildWeeklyPlan(
  ...
  const adaptiveTarget =
    recoveryScore < 45 ? Math.max(1, weeklyTarget - 1) : weeklyTarget;

  const safeTarget = Math.max(1, Math.min(7, adaptiveTarget));
  ...
  const safeRecommendedIndex = Math.max(0, recommendedIndex);

  const ordered = [
    ...ROUTINE.slice(safeRecommendedIndex),
    ...ROUTINE.slice(0, safeRecommendedIndex),
  ];

  const sessions: PlannedWorkout[] = offsets
    .slice(0, safeTarget)
    .map((offset, index) => {
      ...
      const intensity: PlannedWorkout["intensity"] =
        recoveryScore < 55 && index === 0 ? "reduced" : "normal";
  ...
  return {
    ...
    adaptation:
      recoveryScore < 45
        ? "Se ha reducido una sesión por recuperación baja."
        : recoveryScore < 55
          ? "La primera sesión se propone con volumen reducido."
          : "Plan estándar según tu objetivo semanal.",
  };
}
